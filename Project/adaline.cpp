...
void Adaline::inputCheck(double ** input) {
...
void Adaline::calculate(double ** input) {
...
void Adaline::train(double ** input) {
  for(iteration = 1; iteration <= TRAININGSIZE; iteration++) {
    calculate(input);
    error = -2*(input[NOINPUTS][iteration-1] - tot)*input;
    activation();
...
void Adaline::freeBuffer(double ** input) {
  toFile();
...
void Adaline::verify(double ** input) {
  for(iteration = 1; iteration <= TRAININGSIZE; iteration++) {
    calculate(input);
    activation();
...
void Adaline::classify(double ** input) {
